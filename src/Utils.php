...
class DiscordUtils {
	...
	public static function createMarkdownLink ($text, $url)  {
		...
		return '[' . self::sanitizeText( $text ) . '](' . ($wgDiscordSuppressPreviews ? '<' : '') . self::encodeURL($url) . ($wgDiscordSuppressPreviews ? '>' : '') . ')';
	}
	...
	/**
	 * Sanitize input to prevent it from being parsed as markdown styling
	 */
	public static function sanitizeText( $text, $inCodeBlock = false ) {
		if ( !$inCodeBlock ) {
